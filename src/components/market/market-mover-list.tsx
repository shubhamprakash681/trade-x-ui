...
import type { MarketMoverResponse, MarketTrendResponse } from "@/types/api.types";
...
export function MarketMoverList({ title, items, kind, isLoading, isError }: MarketMoverListProps) {
  return (
    <Card className="p-0">
        ...
        {!isLoading && !isError && items?.map((item) => (
          <Link key={item.symbol} href={`/stocks/${encodeURIComponent(item.symbol)}`} className="flex items-center justify-between gap-3 border-b border-border-primary px-5 py-3 last:border-0 hover:bg-bg-tertiary">
            ...
            <div className="shrink-0 text-right">
              <p className="text-sm font-medium text-text-primary">{formatCurrency(item.price)}</p>
              <p className={`text-xs font-medium ${getPnlColor(item.changePercent)}`}>
                {formatPercent(item.changePercent)}{isMover(item) ? ` · ${formatCurrency(item.changeAmount)}` : ""}
              </p>
            </div>
          </Link>
        ))}
